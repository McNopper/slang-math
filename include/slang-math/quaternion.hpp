...
struct quaternion {
    ...
    [[nodiscard]] static constexpr quaternion identity() noexcept { return {0.f, 0.f, 0.f, 1.f}; }
...
[[nodiscard]] inline constexpr float3 operator*(const quaternion& q, const float3& v) noexcept {
    ...
    const float3 t{2.f * (qv.y * v.z - qv.z * v.y), 2.f * (qv.z * v.x - qv.x * v.z), 2.f * (qv.x * v.y - qv.y * v.x)};
    return {v.x + q.w * t.x + (qv.y * t.z - qv.z * t.y),
