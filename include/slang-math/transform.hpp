...
[[nodiscard]] inline float4x4 lookAt(const float3& eye, const float3& center, const float3& up) noexcept {
...
[[nodiscard]] inline float4x4 inverseLookAt(const float3& eye, const float3& center, const float3& up) noexcept {
...
[[nodiscard]] inline float4x4 inversePerspective(float fovY, float aspect, float zNear, float zFar) noexcept {
...
[[nodiscard]] inline float4x4 perspective(float fovY, float aspect, float zNear, float zFar) noexcept {
    const float f = 1.f / std::tan(fovY * 0.5f);
    const float d = zFar - zNear;
    return {
...
template <> [[nodiscard]] inline float2x2 identity<float2x2>() noexcept {
    return float2x2::identity();
}
template <> [[nodiscard]] inline float3x3 identity<float3x3>() noexcept {
    return float3x3::identity();
}
template <> [[nodiscard]] inline float4x4 identity<float4x4>() noexcept {
    return float4x4::identity();
}
template <> [[nodiscard]] inline quaternion identity<quaternion>() noexcept {
